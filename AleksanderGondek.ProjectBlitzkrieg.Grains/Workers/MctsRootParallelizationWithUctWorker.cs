...
{
    public class MctsRootParallelizationWithUctWorker : Grain, IMctsRootParallelizationWithUtcWorker
    {
        public async Task<IDictionary<string, int>> GetNextMove(ProcessingRequest request)
        {
            ...
            {
                var worker = GrainFactory.GetGrain<IMctsSerialWithUtcWorker>(Guid.NewGuid());
